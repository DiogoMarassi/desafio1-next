...
export default function EditarPratoPage() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...prato,
      alimentos: prato.alimentos.map(a => a.id) // <-- apenas IDs
    };
    try {
      const response = await fetch(`http://localhost:2000/api/pratos/${prato.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`,
  ...
  return (
    <div className="min-h-screen flex flex-row items-center justify-center bg-gray-100 p-6 gap-5">
      <div className="bg-white p-6 rounded-lg shadow-md w-full max-w-4xl">
        <h1 className="text-2xl font-bold mb-4 text-black">Editar Prato #{id}</h1>
          ...
          <div className="flex gap-5 justify-around items-center">
            <div>
              ...
              <input
                type="date"
                className="w-full p-2 border rounded text-black"
                value={prato.data_lancamento.split('T')[0]} // extrai apenas a data
